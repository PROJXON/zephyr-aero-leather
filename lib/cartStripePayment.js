...
export default async function cartStripePayment(req) {
    try {
        ...
        if (billing) syncAddress(paymentIntentObj.billing, billing, woo_order_id, true)
        if (shipping) syncAddress(paymentIntentObj.shipping, shipping, woo_order_id, false)
